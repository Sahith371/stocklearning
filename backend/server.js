  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        ...
        console.log('fluent-ffmpeg not installed, using canvas-based thumbnail generation');
      }
      ...
      if (ffmpeg) {
        ...
        console.log('Generating thumbnail with FFmpeg for:', videoPath);
        ...
        ffmpeg(videoPath)
          ...
          .on('error', (err) => {
            ...
            // Fall through to canvas-based generation
            generateCanvasThumbnail(videoPath, outputPath).then(resolve).catch(() => resolve(null));
          });
      } else {
        // Use canvas-based generation
        const result = await generateCanvasThumbnail(videoPath, outputPath);
        resolve(result);
      }
    } catch (error) {
...
// Canvas-based thumbnail generation (fallback when FFmpeg is not available)
async function generateCanvasThumbnail(videoPath, outputPath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      ctx.fillText(path.basename(videoPath), 400, 320);
      
      // Save to file
      const buffer = canvas.toBuffer('image/jpeg', { quality: 0.9 });
      fs.writeFileSync(outputPath, buffer);
      
      console.log('Canvas thumbnail generated:', outputPath);
      resolve(outputPath);
    } catch (error) {
...
app.get('/myvideos', (req, res) => {
  res.sendFile(path.join(__dirname, '..', 'frontend', 'myvideos.html'));
});
...
app.get('/paper-trading', (req, res) => {
  res.sendFile(path.join(__dirname, '..', 'frontend', 'paper-trading.html'));
});
...
app.post('/api/videos', requireAuth, upload.fields([
  ...
  try {
    ...
    // Generate thumbnail
    const thumbnailFileName = videoFile.filename.replace(/\.[^/.]+$/, '') + '_thumb.jpg';
    const thumbnailPath = path.join(uploadsDir, thumbnailFileName);
    
    // Use absolute path for video file
    const videoFilePath = path.join(uploadsDir, videoFile.filename);
    
    console.log('=== THUMBNAIL GENERATION ===');
    console.log('Video file path:', videoFilePath);
    console.log('Thumbnail path:', thumbnailPath);
    console.log('Video file exists:', fs.existsSync(videoFilePath));
    
    try {
      const result = await generateThumbnail(videoFilePath, thumbnailPath);
      if (result) {
  ...
  try {
    ...
    if (isAuthentic) {
      ...
      if (video) {
        ...
        // Only lookup buyer if userId is a valid ObjectId (not 'anonymous')
        let buyer = null;
...
app.get('/admin-working.html', (req, res) => {
  try {
    const adminPath = path.join(__dirname, '..', 'frontend', 'admin-working.html');
...
app.get('/paper-trading', (req, res) => {
  try {
    const paperTradingPath = path.join(__dirname, '..', 'frontend', 'paper-trading.html');
...
// ====== Dynamic Thumbnail Endpoint ======
// Generate thumbnail on-the-fly for videos without thumbnails
app.get('/api/video-thumbnail/:videoId', async (req, res) => {
  try {
    ...
    // If video has thumbnail, serve it
    if (video.thumbnail) {
    ...
    const videoPath = path.join(__dirname, '..', 'uploads', path.basename(video.videoUrl));
    if (!fs.existsSync(videoPath)) {
      return res.status(404).send('Video file not found');
    }
    
    const thumbnailFileName = `thumb_${video._id}.jpg`;
    ...
    if (result && fs.existsSync(thumbnailPath)) {
      ...
      video.thumbnail = `/uploads/${thumbnailFileName}`;
      await video.save();
      return res.sendFile(thumbnailPath);
    }
    
    // If generation failed, return a better placeholder with video title
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 800 450">
      ...
      <rect fill="url(#grad1)" width="800" height="450"/>
      <circle cx="400" cy="180" r="80" fill="rgba(255,255,255,0.2)"/>
      <text fill="white" font-family="Arial, sans-serif" font-size="60" font-weight="bold" x="400" y="200" text-anchor="middle">${initials}</text>
      <text fill="white" font-family="Arial, sans-serif" font-size="28" x="400" y="320" text-anchor="middle">${title.substring(0, 30)}${title.length > 30 ? '...' : ''}</text>
      <text fill="rgba(255,255,255,0.8)" font-family="Arial, sans-serif" font-size="18" x="400" y="360" text-anchor="middle">▶ Click to Watch</text>
    </svg>`;
    return res.redirect(`data:image/svg+xml,${encodeURIComponent(svg)}`);
...
// ====== Phone OTP System ======
// Temporary storage for phone OTPs (in production, use Redis or similar)
...
app.post('/api/send-phone-otp', async (req, res) => {
  try {
    ...
    if (!phone.match(/^\+[1-9]\d{1,14}$/)) {
      return res.status(400).json({ error: 'Invalid phone format. Use format: +919876543210' });
    }
    ...
    if (twilioSid && twilioToken && twilioPhone) {
      try {
        ...
        await client.messages.create({
          body: `Your StockMaster verification code is: ${otp}. Valid for 10 minutes.`,
          from: twilioPhone,
          to: phone
        });
        ...
        // Continue - we'll return OTP in response for testing
      }
    } else {
    ...
    res.json({ 
      ...
      otp: otp // Return OTP for testing when SMS not configured
    });
...
app.post('/api/verify-phone-otp', (req, res) => {
  try {
    ...
    if (!stored) {
      return res.status(400).json({ error: 'OTP expired or not found. Please request new OTP.' });
    }
